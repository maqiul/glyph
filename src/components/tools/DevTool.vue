...
<template>
  <div class="dev-tool">
    ...
    <div class="dev-body">
      ...
      <!-- Diff -->
      <div v-else-if="active === 'diff'" class="dev-panel">
        <div class="diff-inputs">
          <textarea
            v-model="diffA"
            class="dev-io"
            :placeholder="t('dev.origPh')"
            spellcheck="false"
          ></textarea>
          <textarea
            v-model="diffB"
            class="dev-io"
            :placeholder="t('dev.newPh')"
            spellcheck="false"
          ></textarea>
        </div>
        <div class="dev-actions">
          <button class="btn" @click="swapDiff">{{ t('dev.swap') }}</button>
          <button class="btn" @click="clearDiff">{{ t('dev.clear') }}</button>
      ...
      <!-- JSON → TS -->
      <div v-else-if="active === 'json2ts'" class="dev-panel">
        <textarea
          v-model="tsIn"
          class="dev-io"
          :placeholder="t('dev.jsonPh')"
          spellcheck="false"
          style="min-height: 140px; flex: none"
        ></textarea>
        <div class="dev-actions">
          <label class="cfg-label">{{ t('dev.rootName') }}</label>
          <input v-model="tsRoot" class="dev-line" spellcheck="false" style="max-width: 160px" />
          ...
          <button class="btn" :disabled="!tsOut" @click="copy(tsOut)">{{ t('dev.copy') }}</button>
          <button
            class="btn"
            :disabled="!tsOut"
